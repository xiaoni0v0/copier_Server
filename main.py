...
VERSION: Dict[bytes, Tuple[int, int, int]] = {
    bytes([0b10000101]): (2, 1, 1)
}
...
class App:
    ...
    def __init__(self):
        ...
        self.frame1.grid(row=0, column=0, sticky='nesw', padx=10, pady=10)
        # 上方提示（标签）
        ...
        self.frame2.grid(row=0, column=1, sticky='nesw', padx=10, pady=10)
        # 上方标签
    ...
    def server_main(self):
        while True:
            try:
                ...
                self.ts.append(threading.Thread(target=self.treat.main, args=ci))
                self.ts[-1].start()
...
class Treat:
    ...
    def __init__(self, app_obj: 'App'):
        ...
        self.ver_map: Dict[Tuple[int, int, int], Callable[[socket.socket], int]] = {
            (2, 1, 1): self.version_2_1
        }
    ...
    def o(self, formatter: str, *args):
        ...
        o: output
        """
        return self.app_obj.write1('<%s> %s' % (threading.current_thread().name, formatter % args))
    ...
    def main(self, cs: socket.socket, ca: Tuple[str, int]):  # cs: client_socket, ca: client_addr
        ...
        self.o('-> version: %s\n', ver)
        if ver in VERSION:
            if self.ver_map[VERSION[ver]](cs) == 0:
                self.o('已完成通信!\n%s\n', '-' * 40)
        else:
            self.o('版本不支持，已断开连接\n%s\n', '-' * 40)
        cs.close()

    def version_2_1(self, cs: socket.socket) -> int:
        try:
            # 1 B -> what
            # 8 B -> sk
